...
sub _ReplaceTicketAttributes {
    ...
    if ( $Param{Field} =~ m{<OTRS_CUSTOMER_DynamicField_(\S+?)>} ) {
        my $DynamicFieldName = $1;
        my $DynamicFieldConfig = $DynamicFieldObject->DynamicFieldGet(
            Name => $DynamicFieldName,
        );
        my $DisplayValue = $DynamicFieldBackendObject->ValueGet(
            DynamicFieldConfig => $DynamicFieldConfig,
            ObjectName         => $Param{Ticket}->{"CustomerUserID"},
        );
        my $DisplayValueStrg = $DynamicFieldBackendObject->ReadableValueRender(
            DynamicFieldConfig => $DynamicFieldConfig,
            Value              => $DisplayValue,
        );
        $Param{Field} =~ s{<OTRS_CUSTOMER_DynamicField_$DynamicFieldName>}{$DisplayValueStrg->{Value} // ''}ige;
    }
